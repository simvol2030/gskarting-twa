<script lang="ts">
	...
	const handleFileUpload = (e: Event) => {
		...
		reader.onload = (e) => {
			...
			// SECURITY FIX #2 (IMPROVED): Prevent path traversal attacks
			// Step 1: Remove .. sequences and path separators
			const cleanName = file.name.replace(/\.\./g, '').replace(/[\/\\]/g, '');
			// Step 2: Sanitize remaining characters (only alphanumeric, dot, dash, underscore)
			...
			formData.image = `/uploads/promotions/${timestamp}_${sanitizedName}`;
		};
